...
void display()
{
    if (++display_count <= 100000)
    {
        ...
        glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);TEST_OPENGL_ERROR(); // FILL or LINE to debug vertices and indices (backface culling)
...
void add_floor(float floor_size)
{
    ...
    glm::vec3 position0 = flame_particle_effect.g_position - Y + (-X - Z) * floor_size;  
    glm::vec3 position1 = flame_particle_effect.g_position - Y + (X - Z) * floor_size;
    glm::vec3 position2 = flame_particle_effect.g_position - Y + (-X + Z) * floor_size;
    glm::vec3 position3 = flame_particle_effect.g_position - Y + (X + Z) * floor_size;
...
{
    std::string file = "plane10.obj";
    if (!load_obj(file, vertices, indices))
    {
        std::cerr << "Error while loading file" << std::endl;
        return -1;
    }
    displace_vertices(glm::vec3(0, flame_particle_effect.g_position.y, flame_particle_effect.g_position.z));
...
bool init_POV()
{
    ...
    std::cout << "fire_program_id = " << fire_program_id << std::endl;
    glUseProgram(fire_program_id);
    ...
    glUniform3fv(light_pos, 1, glm::value_ptr(flame_particle_effect.g_position + glm::vec3(0, 30, 0)));TEST_OPENGL_ERROR();
...
bool init_textures()
{
    ...
    auto texture = stbi_load("textures/forest/1K-forest_albedo.jpg", &width, &height, &bpp, 3);
    if (texture == 0)
        return false;
