...
std::vector<zone> smoke_wind_zones;

SmokeEffect::SmokeEffect(glm::vec3 position,
...
{
    force = glm::vec3(0, 2, 0); // Smoke particles elevation
...
void SmokeEffect::update(float fDeltaTime)
{
    ...
    if (add_wind && smoke_wind_zones.size() < max_wind_zones)
    {
        ...
        smoke_wind_zones.push_back(std::make_pair(wind_lim, wind_dir));
    }
    bool remove_wind = random_range(0, 1) < wind_chances; 
    if (remove_wind && smoke_wind_zones.size() != 0)
    {
        std::cout << "removed wind !" << std::endl;
        int index = random_range(0, smoke_wind_zones.size());
        smoke_wind_zones.erase(smoke_wind_zones.begin() + index);
    }
    ...
    for (auto &particle : particles)
    {
        ...
        for (auto &wind_zone : smoke_wind_zones)
        {
        ...
        particle.color.x = 1 * (particle.lifeTime - particle.age) / particle.lifeTime; // to fix - experiment
        particle.color.y = 1 * (particle.lifeTime - particle.age) / particle.lifeTime; // to fix - experiment
        particle.color.z = 1 * (particle.lifeTime - particle.age) / particle.lifeTime; // to fix - experiment
        particle.color.w = fclamp(1 * (particle.age / particle.lifeTime), 0, 1);
